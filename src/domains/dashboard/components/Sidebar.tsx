...
export default function Sidebar() {
  return (
    <aside className="flex flex-col items-center py-6 px-2 bg-gray-50 rounded-r-3xl min-h-screen w-16">
      <div className="flex flex-col items-center gap-6 flex-1">
        {icons.map((icon) => (
          <button
            key={icon.name}
            className="p-3 rounded-xl hover:bg-gray-200 transition-colors text-gray-500 hover:text-gray-700"
            title={icon.name}
          >
      ...
      <button className="p-3 rounded-xl bg-red-50 text-red-500 hover:bg-red-100 transition-colors mt-auto">
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
